...
#include <ql/method/montecarlo/lsmbasissystem.hpp>
#include <ql/cashflows/fixedratecoupon.hpp>
#include <ql/process/hullwhiteprocess.hpp>
...
namespace QuantLib {
	...
	PowerSpreadPricer::PowerSpreadPricer(
		Size assetNumber,
	...
	IREarlyExcercisePathPricer(assetNumber, callValue, processes, cashflows, polynomOrder, polynomType),
	...
	Real PowerSpreadPricer::cpnCalculate(const MultiPath& path, Size idx) const {
		   ...
		   if (isAvg_) { 
			   ...
			   for (Size i = 0; i < n; ++i) {
				...
				if (idx == 0)
					...
					cpnRate = cpn -> spread() + cpn -> gearing() * (indexCalculator_(t, Period(Quarterly), path[1][idx]) - indexCalculator_(t, Period(Quarterly), path[2][idx]));
					cpnValue = cpnRate * firstT / deltaT;	
			}
		...
		else if(boost::shared_ptr<FixedRateCoupon> fixedcpn = boost::dynamic_pointer_cast<FixedRateCoupon>(cashflows_[idx + cashflows_.size() - path.pathSize()])){
